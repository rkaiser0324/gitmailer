...
class Git
{
    ...
    public function __construct($repo, $git_path, $project=null)
    {
        $this->repo = $repo;
    ...
    {
	$cmd = sprintf($this->git_path.' --git-dir=%s config --get %s',
                           escapeshellarg($this->repo),
		escapeshellarg($key));

        return self::exec('IDF_Scm_Git::getBranches',
                   $cmd, $out, $return);
    }
	...
	$cmd = sprintf($this->git_path.' --git-dir=%s branch',
                           escapeshellarg($this->repo));
    ...
    {
	$cmd = sprintf($this->git_path.' --git-dir=%s cat-file -t %s',
                           escapeshellarg($this->repo),
			    escapeshellarg($hash));
        $ret = 0; $out = array();
	...
	$cmd = sprintf($this->git_path.' --git-dir=%s  ls-tree -l %s %s',
                           escapeshellarg($this->repo),
			   escapeshellarg($tree), 
...
//                     escapeshellarg($tree), escapeshellarg($folder));
    ...
    {
        $cmd = sprintf($this->git_path.' --git-dir=%s  ls-tree -r -t -l %s',
                       escapeshellarg($this->repo),
    ...
    {
	$cmd = sprintf($this->git_path.' --git-dir=%s  cat-file blob %s',
                       escapeshellarg($this->repo),
                       escapeshellarg($def->hash));
	
        return ($cmd_only)
            ? $cmd : self::exec('IDF_Scm_Git::getFile', $cmd);
    }
    ...
    {
        if ($getdiff) {
            $cmd = sprintf($this->git_path.' --git-dir=%s show --date=iso --pretty=format:%s %s',
                           escapeshellarg($this->repo),
                           "\"".$this->mediumtree_fmt."\"",
                           escapeshellarg($commit));
        } else {
            $cmd = sprintf($this->git_path.' --git-dir=%s show -1 --date=iso --pretty=format:%s %s',
                           escapeshellarg($this->repo),
                           "\"".$this->mediumtree_fmt."\"",
                           escapeshellarg($commit));
        }
	//$cmd = $this->git_path . ' --git-dir ' . escapeshellarg($this->repo) . ' log';
        $out = array();
        //$cmd = Pluf::f('idf_exec_cmd_prefix', '').$cmd;
    ...
    public function getChangeLog($commit='HEAD', $n=10)
    {
        ...
        $cmd = sprintf('GIT_DIR=%s '.$this->git_path.' log%s --date=iso --pretty=format:\'%s\' %s',
